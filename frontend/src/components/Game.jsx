...
export default function EyeBlinkFlappyBirdDOM() {
  const { videoRef, isRightEyeClosed } = useRightEyeControl();
  ...
  const [pipes, setPipes] = useState([
    { x: gameWidth, height: 200, scored: false },
  ]);
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      // Bird
      if (isRightEyeClosed) setVelocity(JUMP);
      else setVelocity((v) => v + GRAVITY);
      ...
      setPipes((prev) => {
        let updated = prev.map((p) => {
          ...
          if (!p.scored && newX + pipeWidth < birdX) {
            setScore((s) => s + 1);
            p.scored = true;
          }
        ...
        if (updated[updated.length - 1].x < gameWidth / 2) {
          updated.push({
            x: gameWidth,
            height: randomHeight(),
            scored: false,
  ...
  }, [running, isRightEyeClosed, velocity]);

  return (
    <div style={{ textAlign: "center" }}>
      <h2>DOM Flappy Bird (Blink)</h2>
      <p>Score: {score}</p>
